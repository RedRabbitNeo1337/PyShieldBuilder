...
class _InMemoryLoader(importlib.abc.Loader):
    def __init__(self, module_name: str, source: str, *, code_cache: dict[str, CodeType]) -> None:
    ...
    def exec_module(self, module: ModuleType) -> None:
        ...
        exec(code, module.__dict__)  # type: ignore[arg-type]
...
class _InMemoryFinder(importlib.abc.MetaPathFinder):
    def __init__(self, module_map: dict[str, str], package_names: set[str]) -> None:
        ...
        self._code_cache: dict[str, CodeType] = {}
    ...
    def find_spec(
        ...
        return importlib.util.spec_from_loader(
            fullname, loader, is_package=fullname in self._package_names
        )
...
def _validate_runtime_environment() -> None:
    ...
    if (
        builtins.open is not _ORIGINAL_OPEN
        or importlib.import_module is not _ORIGINAL_IMPORT_MODULE
    ):
...
def execute_package(package_path: str, password: str, *, entrypoint: str | None = None) -> Any:
    ...
    package_names = {
        _module_name_from_path(path) for path in source_map if path.endswith("__init__.py")
    }
